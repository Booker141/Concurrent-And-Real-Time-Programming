...
public class calculopiParalelomultiCont implements Runnable {
	...
	*/
	public calculopiParalelomultiCont(int n){
		this.veces=n;
		this.r=new Random();
	}
	/**
	...
	*/
	public void run(){
		...
		for(int i=0;i<veces;i++){
			Ax=r.nextDouble();
			Ay=r.nextDouble();
			if(Math.pow(Ax, 2)+Math.pow(Ay, 2)<=1)conthilo++;
		}
		synchronized(o){
			cont=cont+conthilo;
		}
	...
	*/
	...
	public static void main(String[] args)throws Exception{
		...
		for(int i=0;i<nt;i++){
			exe.execute(new calculopiParalelomultiCont(n/nt));
		}
		exe.shutdown();
