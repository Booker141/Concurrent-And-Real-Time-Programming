...
donde x es la letra que queremos cifrar (representada por su c´odigo ASCII o
...
public class desCesar {
    ...
    public static String descifrar(String c,int n){
		...
		for(int i=0;i<c.length();i++){
			...
			if(letra>95){
				letra = letra-97;
				if(letra<n){
				letra=((letra+26-n)%26)+97;
				}
				else letra =(letra-n)%26+97;
				
			}
			else{
					letra = letra-65;
					if(letra<n){
					letra =((letra+26-n)%26)+65;
					}
					else letra =(letra-n)%26+65;
			}
			aux[i]=(char)letra;
	...
	public static void main(String[] args) {
		...
		System.out.println("Introduzca la cadena a descifrar: ");
		cadena = scan.nextLine();
		
		System.out.println("Introduzca un valor n para el descifrado cesar: ");
		n=scan.nextInt();
	
		System.out.println("Cadena descifrada: " + descifrar(cadena,n) );
	}
